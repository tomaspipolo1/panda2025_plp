...
interface FiltrosPercepcionesProps {
  onFilter: (filters: any) => void
}

export function FiltrosPercepciones({ onFilter }: FiltrosPercepcionesProps) {
  const [tipoPercepcion, setTipoPercepcion] = useState<string>("todos")
  ...
  const handleLimpiar = () => {
    setTipoPercepcion("todos")
    setJurisdiccion("todas")
    setFechaDesde("")
    setFechaHasta("")
    onFilter({
      tipoPercepcion: "todos",
      jurisdiccion: "todas",
      fechaDesde: "",
      fechaHasta: "",
    })
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-6 mb-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {/* Tipo de Percepción */}
        <div>
          <label htmlFor="tipoPercepcion" className="block text-sm font-medium text-gray-700 mb-1">
            Tipo de Percepción
          </label>
          <Select value={tipoPercepcion} onValueChange={setTipoPercepcion}>
            <SelectTrigger id="tipoPercepcion" className="w-full">
              <SelectValue placeholder="Todos" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="todos">
                <div className="flex items-center">
                  <Checkbox checked={tipoPercepcion === "todos"} className="mr-2" />
                  Todos
                </div>
              </SelectItem>
