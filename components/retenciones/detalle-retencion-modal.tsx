...
interface DetalleRetencionProps {
  isOpen: boolean
  onClose: () => void
  retencion: {
    id: string
...
export function DetalleRetencionModal({ isOpen, onClose, retencion }: DetalleRetencionProps) {
        ...
        <DialogHeader>
          <DialogTitle className="text-xl font-bold flex items-center justify-between">
            <div className="flex items-center">
              <span>Detalle de Retención - {retencion.comprobante}</span>
            </div>
        ...
        <div className="mt-4">
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              ...
              <p className="font-medium">{retencion.fecha}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="font-medium">{retencion.jurisdiccion}</p>
            </div>
            <div>
              <p className="text-sm text-gray-500">Comprobante</p>
              <p className="font-medium">{retencion.comprobante}</p>
            </div>
            <div>
              <p className="text-sm text-gray-500">Concepto</p>
              <p className="font-medium">{retencion.concepto}</p>
            </div>
            <div>
              <p className="text-sm text-gray-500">Base Imponible</p>
              <p className="font-medium">{formatCurrency(retencion.baseImponible)}</p>
            </div>
            <div>
              <p className="text-sm text-gray-500">Alícuota</p>
              <p className="font-medium">{formatPercentage(retencion.alicuota)}</p>
            </div>
            <div>
              <p className="text-sm text-gray-500">Monto</p>
              <p className="font-medium">{formatCurrency(retencion.monto)}</p>
            </div>
          </div>

          <Separator className="my-4" />

          {retencion.detalles?.items && retencion.detalles.items.length > 0 && (
            <>
              ...
              <div className="overflow-x-auto">
                <table className="w-full border-collapse">
                  ...
                  <tbody>
                    {retencion.detalles.items.map((item, index) => (
          ...
          {retencion.detalles?.observaciones && (
            <div className="mt-4">
              <p className="text-sm text-gray-500">Observaciones</p>
              <p className="mt-1 p-2 bg-gray-50 rounded-md">{retencion.detalles.observaciones}</p>
            </div>
          )}
        </div>
      </DialogContent>
