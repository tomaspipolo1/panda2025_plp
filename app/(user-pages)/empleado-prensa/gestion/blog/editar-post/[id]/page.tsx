...
// Función para simular la obtención de datos del post
const getPostById = (id: string) => {
  // En una implementación real, esto sería una llamada a la API
  return {
    id,
    titulo: "Puerto La Plata recibe su primer crucero de la temporada",
    subtitulo: "Un hito importante para el turismo local y la economía regional",
    fecha: "2023-11-15",
    hora: "09:30",
    cuerpo:
      "El Puerto La Plata recibió hoy el primer crucero de la temporada 2023-2024, marcando un hito importante para el turismo local. El crucero, con más de 2,000 pasajeros a bordo, atracó en la terminal de pasajeros recientemente renovada.\n\nLas autoridades portuarias y representantes del gobierno local dieron la bienvenida a los visitantes con un evento especial que incluyó presentaciones culturales y degustaciones de productos regionales.\n\nEste arribo forma parte de una estrategia más amplia para posicionar a La Plata como un destino clave en las rutas de cruceros del Atlántico Sur, lo que se espera genere un impacto económico significativo en la región.",
    categoria: "noticias",
    imagenPrincipal: "/cruise-ship-arrival.png",
    galeria: [
      {
        id: "img1",
        url: "/cruise-disembarkation.png",
        epigraph: "Pasajeros desembarcando en el Puerto La Plata",
      },
      {
        id: "img2",
        url: "/cruise-arrival-celebration.png",
        epigraph: "Celebración de bienvenida organizada por autoridades locales",
      },
      {
        id: "img3",
        url: "/la-plata-cruise-arrival.png",
        epigraph: "Vista panorámica del crucero en el puerto",
      },
    ],
  }
}
...
export default function EditarPostPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    setFormData({
      ...
      hora: postData.hora,
      cuerpo: postData.cuerpo,
      categoria: postData.categoria,
    })

    setMainImage({
      file: null,
      preview: postData.imagenPrincipal,
    })

    // Convertir la galería al formato requerido
    const galleryData = postData.galeria.map((img) => ({
      id: img.id,
      file: null,
      preview: img.url,
  ...
  return (
    <div className="container mx-auto py-6">
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{publishingStatus === "loading" ? "Actualizando post..." : "Post actualizado"}</DialogTitle>
          </DialogHeader>
            ...
            {publishingStatus === "loading" ? (
              <div className="flex flex-col items-center">
                ...
                <p className="mt-4 text-center text-sm text-gray-500">
                  Estamos actualizando su post, por favor espere...
                </p>
              </div>
            ) : (
              <div className="flex flex-col items-center">
                ...
                <p className="mt-4 text-center text-sm text-gray-500">¡Su post ha sido actualizado exitosamente!</p>
              </div>
            )}
          </div>
        </DialogContent>
